    ...
    data() {
        return {
            ...
            rules: {
                ...
                workFlowNodeDesc: [{ required: true, message: "请输入流程节点描述", trigger: "blur" }],
                workFlowNodeType: [{ required: true, message: "请选择节点层级", trigger: "blur" }],
                workFlowNodeName :[{ required: true, message: "请输入流程节点名称", trigger: "blur" }],
    ...
    methods: {
        ...
        Add(addForm){
            this.$refs[addForm].validate(valid => {
                if (valid) {
                    this.$axios
                        .post("/api/workflowNodeadd", {
                            workFlowCode: this.addForm.workFlowCode,//菜单名称
                            workFlowNodeType: this.addForm.workFlowNodeType,
                            workFlowNodeName: this.addForm.workFlowNodeName,
                            workFlowNodeDesc: this.addForm.workFlowNodeDesc,
        ...
        dateformatUpdateTime: function (row, column) {
            let d = new Date(row.updateTime.substr(0, 19));//加入substr(0, 19)处理兼容ios报错NAN
            let year = d.getFullYear();       //年
            let month = d.getMonth() + 1;     //月
            let day = d.getDate();            //日
            let hh = d.getHours()+8;            //时
        ...
        search(){
            this.$axios
                .post("/api/workflowNodeList", {
                    workFlowNodeCode:this.params.workFlowNodeCode,
                    workFlowNodeName: this.params.workFlowNodeName,
                ...
                this.$axios
                    ...
                    .then(successResponse => {
                        if (successResponse.data.status === 200) {
                            ...
                            this.initWorkFlowNodeType();
                            //初始化工作流列表信息
                           this.initWorkFlowList('edit');
                        }
                        if (successResponse.data.status === 400) {
                            let warnMessage = successResponse.data.description;
                            this.$message({
        ...
        initWorkFlowNodeType(){  //初始化字典值workflowNode
            this.$axios
                .post("/api/workflowNodeAddInit", {
                ...
                .then(successResponse => {
                    if (successResponse.data.status === 200) {
                        this.workFlowNodeTypeList=[];
                        this.workFlowNodeTypeList=successResponse.data.data.workFlowNodeTypeList;
        ...
        initWorkFlowList(actionType){
            this.$axios
                ...
                .then(successResponse => {
                    if (successResponse.data.status === 200) {
                        this.workFlowList=[];
                        this.workFlowList=successResponse.data.data.grid.list;
                        this.pageWorkFlowListParms.total=successResponse.data.data.grid.total;
                        if(actionType=='add'){
                            this.dialogFormVisible = true;
                        }else {
                            this.updateFormVisible=true;
        ...
        Update(updateForm){
            this.$refs[updateForm].validate(valid => {
                if (valid) {
                    this.$axios
                        .post("/api/workflowNodeUpdate", {
                            workFlowCode:this.updateForm.workFlowCode,
                            workFlowNodeCode: this.updateForm.workFlowNodeCode,
                            workFlowNodeType: this.updateForm.workFlowNodeType,
